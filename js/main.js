...
function startTimeUpdate(locationData) {
    const timezoneLabel =
        typeof locationData === "string"
            ? locationData
            : locationData?.timezone || "UTC";
    const timezoneId =
        typeof locationData === "object" ? locationData.timezoneId : undefined;
    ...
    function updateTime() {
        ...
        if (timeElement || mobileTimeElement) {
            ...
            if (timezoneId) {
                ...
                typeof timezoneLabel === "string" &&
                timezoneLabel.toUpperCase().startsWith("UTC")
            ) {
                const offsetText = timezoneLabel
                    .toUpperCase()
                    .replace("UTC", "")
                    .trim();
                const offsetValue = parseFloat(offsetText) || 0;
                const utcTime =
                    now.getTime() + now.getTimezoneOffset() * 60 * 1000;
                const targetTime = new Date(
                    utcTime + offsetValue * 60 * 60 * 1000
                );
                timeString = targetTime.toLocaleTimeString("en-US", {
                    hour: "numeric",
                    minute: "2-digit",
                    hour12: true,
                });
...
function updateSocialLinks(social = {}) {
    ...
    updateLink('[data-social="linkedin"]', social.linkedin);
    updateLink('[data-social="github"]', social.github);
}
...
function updateContent() {
    ...
    if (config && config.header) {
        ...
        if (config.header.location) {
            ...
            startTimeUpdate(config.header.location);
        }
    ...
    if (contact) {
        ...
        if (contactEmail && contact.email) {
            ...
            if (contactInfo) {
                contactInfo.style.removeProperty("display");
            }
            contactEmail.textContent = trimmedEmail;
            // Ensure the href uses a mailto scheme when an email address is provided
            if (trimmedEmail.startsWith("mailto:") || trimmedEmail.startsWith("http")) {
                contactEmail.href = trimmedEmail;
            } else if (trimmedEmail.includes("@")) {
                contactEmail.href = `mailto:${trimmedEmail}`;
            } else {
                contactEmail.href = trimmedEmail || "#";
            }
            contactEmail.setAttribute("aria-label", `Email ${trimmedEmail}`);
        } else if (contactInfo) {
            // Hide the email row entirely if no email is provided
        ...
        try {
            ...
            if (ctasContainer) {
                ...
                function createCta(btnData, primary = false) {
                    ...
                    if (btnData.color) {
                        ...
                        if (
                            ...
                            const hoverBorder =
                                btnData.hoverBorderColor || hoverBackground;
                ...
                const hasCtas = ctasContainer.childElementCount > 0;
                ctasContainer.setAttribute("aria-hidden", hasCtas ? "false" : "true");
            }
        } catch (e) {
...
function renderProjects(projects) {
    ...
    projects.forEach((project, index) => {
        ...
        const technologiesMarkup = project.technologies
        ...
        if (isMobile) {
            // Mobile version - present content without relying on hover states
            projectCard.classList.add("project-card--mobile");
            projectCard.innerHTML = `
                ...
                <div class="project-details">
                    <div class="project-details-header">
                        ...
                        ${project.year
                    ? `<span class="project-year">Year: ${project.year}</span>`
                    : ""
                }
                    </div>
                    ${projectDescription
                    ...
                    <span class="project-card-cta">Tap to explore this project</span>
                </div>
            `;
        } else {
        ...
        projectCard.setAttribute(
            "aria-label",
            `View project ${project.title || "details"}`
        );
...
function populateModalContent(project) {
    ...
    if (techContainer) {
        ...
        if (technologies.length === 0) {
            ...
            placeholder.className = "tech-tag";
            placeholder.textContent = "Tech stack coming soon";
            techContainer.appendChild(placeholder);
        } else {
            technologies.forEach((tech) => {
                const isStringTech = typeof tech === "string";
                const techName = isStringTech ? tech : tech?.name;
                const techIconClass = !isStringTech ? tech?.icon : null;

                if (techIconClass) {
                    const techIcon = document.createElement("div");
                    techIcon.className = "tech-icon";
                    techIcon.setAttribute(
                        "data-tooltip",
                        techName || "Technology"
                    );
                    ...
                    icon.className =
                        techIconClass || "devicon-javascript-plain";
                    ...
                    techTag.textContent = techName || "Technology";
    ...
    if (ctaContainer) {
        ...
        const addCta = (cta) => {
            ...
            // Avoid duplicates except for placeholders
            ...
            ctas.push({
                ...
                href,
                icon: cta.icon || null,
                variant: cta.variant || null,
            });
        };

        if (Array.isArray(project.cta)) {
            project.cta.forEach((ctaItem, index) => {
                addCta({
                    text: ctaItem?.text,
                    link: ctaItem?.link,
                    icon: ctaItem?.icon,
        ...
        if (
            project.link &&
            project.link !== "#" &&
            !seenLinks.has(normalizeLink(project.link))
        ) {
            addCta({
                text: "View Project",
                link: project.link,
        ...
        if (
            project.github &&
            !seenLinks.has(normalizeLink(project.github))
        ) {
            addCta({
                text: "GitHub",
                link: project.github,
        ...
        ctaContainer.setAttribute("aria-hidden", ctas.length === 0 ? "true" : "false");

        if (ctas.length === 0) {
            ctaContainer.style.display = "none";
        } else {
            ctaContainer.style.removeProperty("display");
            ctas.forEach((cta, index) => {
                const button = document.createElement("a");
                button.classList.add("project-cta-btn");
                button.classList.add(
                    cta.variant || (index === 0 ? "primary" : "secondary")
                );
                const href = cta.href || "#";
                button.href = href;
                ...
                label.className = "project-cta-text";
                label.textContent = cta.text;
